...
const FALLBACK_UNIVERSITIES = [
    {
        ...
        slug: "swarrnim-startup-and-innovation-university",
    ...
    },
    {
        ...
        slug: "sankalchand-patel-university",
    ...
    },
    {
        ...
        slug: "ajeenkya-d-y-patil-university",
    ...
    },
    {
        ...
        slug: "karpaga-vinayaga-college-of-engineering-and-technology",
...
const getAllUniversities = async (req, res) => {
    try {
        let universities = await University.find().sort({ rating: -1, globalRanking: 1 });
        ...
        console.error('Error fetching universities from DB, returning 6 specified universities fallback:', error);
        res.json({
            success: true,
            universities: FALLBACK_UNIVERSITIES
        });
...
const getUniversityById = async (req, res) => {
    try {
        ...
        if (!university) {
            ...
            university = FALLBACK_UNIVERSITIES.find(u => 
                u._id === idOrSlug ||
                u.slug.toLowerCase() === clean || 
                clean.includes(u.slug.toLowerCase().split('-')[0]) ||
                u.name.toLowerCase().includes(clean.replace(/-/g, ' '))
            );
        }
        ...
        console.error('Error fetching university details, using fallback:', error);
        const clean = req.params.id.toLowerCase();
        const university = FALLBACK_UNIVERSITIES.find(u => 
            u._id === req.params.id ||
            u.slug.toLowerCase() === clean ||
            u.name.toLowerCase().includes(clean.replace(/-/g, ' '))
